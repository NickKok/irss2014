...
#clustering function the SM matrix of each user , number of clusters
...
#parameters
#1-min number of annotation per scene
#2-max/min number of the similar user's that we want the progmam print at the end
#3-path(can be omitted)
def readDataSimilarities(minNumOfAnnotators,MaxSimilarUsers ,path):
	...
	#read the names  of the Scenes and the user's names
	...
	#to create a numpy array for each user
	...
	#read again the file of annotations and put the values at depth array at and construct SM of each user
	...
	#calculate the Nu Nv NUV Muv for each user 
	...
	#find similar users depend of the number of the input
    ...
    #find dissimilar users depend of the number of the input
	...
	#convert the SM of SumofAnnotation to a json file 
	for i in range(len(Scenes)):
		for j in range(len(Scenes)):
...
def main(argv):
	if (len(argv)==2):
		readDataSimilarities(int(argv[1]),int(argv[2]),"")
		statistics()

	if (len(argv)==3):
		readDataSimilarities(int(argv[1]),int(argv[2]),argv[3])
		statistics()
